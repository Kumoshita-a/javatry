...
public class Step07ExceptionTest extends PlainTestCase {
    ...
    public void test_exception_translation_improveChallenge() {
        ...
        // #1on1: 例外の翻訳しっかりできている。(2025/12/09)
        // 実際のwebサービスのアプリ実装で、例外の翻訳がされていることは少ないのが現状。
        // 毎回翻訳やると大変だし、ここぞってところでやるっていうのが判断が難しい(かなり経験が必要)。
        // なので、なかなかアプリ実装で、デバッグのしやすさを考えて例外の翻訳をすることは少ない。
        // ただ、フレームワークの高度な情報提供があることで、なんとかマシになっている。
        // (LastaFluteとDBFluteの例を出してみて)
        
        // #1on1: いつ例外の翻訳をするのか？の基準 (2025/12/09)
        // jflute個人的には...とっさに思いつくところで言うと...
        // o まずレイヤーの接地面で
        // o 情報を集約するなどして、トップに近いところで全情報を出すように翻訳することも (若干苦肉の策)
        // o あと、得体のしれないライブラリを呼ぶとき。念のための例外翻訳
        // o あとあと、フレームワークの場合、利用者がよく発生させるだろうエラーを想像して、ピンポイントで例外翻訳
        //
        // 一番大事なのは、想像。エラーが起きそうだな、とか、起きたときにデバッグつらそうだな、
        // ってのを想像しながら実装してるという感覚がある。
        //
        // エラーが起きたとき、問題を解決しちゃう前に、先に例外を改善して、それからデバッグ始まる。
